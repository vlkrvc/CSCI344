...
class Following(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("user_id", "following_id"),)
    ...
    following = db.relationship(
        "User", backref="following", lazy=False, foreign_keys="Following.following_id"
    )
    ...
    def to_dict_following(self):
        return {"id": self.id, "following": self.following.to_dict()}

    def to_dict_follower(self):
        return {"id": self.id, "follower": self.follower.to_dict()}
